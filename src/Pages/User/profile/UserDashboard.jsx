    ...
    <div className={`min-h-screen ${'bg-slate-100'} p-8`}>
      <div className="">
        ...
        <div className="mb-8">
          ...
          <p className={'text-slate-600'}>Here's what's happening with your learning today</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat, index) => (
            <div
              key={index}
              className={`${'bg-white border-slate-200'} border rounded-2xl p-6 hover:border-cyan-500/50 transition-all duration-300`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Progress Circle */}
          <div className={`${'bg-white border-slate-200'} border rounded-2xl p-6`}>
          ...
          <div className={`${'bg-white border-slate-200'} border rounded-2xl p-6`}>
            ...
            <div className="space-y-4">
              {recentActivity.map((activity, index) => (
                <div 
                  key={index} 
                  className={`flex items-start gap-3 p-3 rounded-xl ${'bg-slate-100 hover:bg-slate-200'} transition-all`}
                >
          ...
          <div className={`${'bg-white border-slate-200'} border rounded-2xl p-6`}>
            ...
            <div className="space-y-3">
              {upcomingClasses.map((cls, index) => (
                <div 
                  key={index}
                  className={`flex items-center gap-3 p-3 rounded-xl ${'bg-slate-100 hover:bg-slate-200'} transition-all`}
                >
                  <div className={`w-3 h-12 ${cls.color} rounded-full`}></div>
                  <div className="flex-1">
                    <p className={`font-semibold ${'text-slate-800'} text-sm`}>{cls.subject}</p>
                    <p className={`text-xs ${'text-slate-500'}`}>{cls.time} • {cls.day}</p>
                  </div>
        ...
        {/* Course Progress */}
        <div className={`mt-6 ${'bg-white border-slate-200'} border rounded-2xl p-6`}>
          <h3 className={`text-xl font-bold ${'text-slate-900'} mb-6 flex items-center gap-2`}>
